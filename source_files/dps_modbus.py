...
#--- new ---
	...
	def voltage_set(self, RWaction='r', value=0.0):	# ! R/W
		return self.function(0x08, self.limits.decimals_vset, RWaction, value, self.limits.voltage_set_max, self.limits.voltage_set_min) # reg_addr, decimal_places, RWaction, value, max_value, min_value 

	def current_set(self, RWaction='r', value=0.0):	# ! R/W
		return self.function(0x09, self.limits.decimals_iset, RWaction, value, self.limits.current_set_max, self.limits.current_set_min) # !reg_addr, decimal_places, RWaction, value, max_value, min_value 

	def voltage(self):	# R
		return self.function(0x0A, self.limits.decimals_v) # !

	def current(self):	# R
		return self.function(0x0B, self.limits.decimals_i)  # !

	def power(self):	# R
		return self.function(0x0D, self.limits.decimals_power)  # !

	def voltage_in(self):	# R
		return self.function(0x0E, self.limits.decimals_vin)  # !

	def lock(self, RWaction='r', value=0):	# R/W
		return self.function(0x0F, 0, RWaction, value, self.limits.lock_set_max, self.limits.lock_set_min) # *reg_addr, decimal_places, RWaction, value, max_value, min_value

	def protect(self):	# R
		return self.function(0x07, 0)

	def cv_cc(self):	# R
		return self.function(0x11, 0)

	def onoff(self, RWaction='r', value=0):	# R/W
		return self.function(0x12, 0, RWaction, value, self.limits.onoff_set_max, self.limits.onoff_set_min) # reg_addr, decimal_places, RWaction, value, max_value, min_value
		
	def b_led(self, RWaction='r', value=0):	# R/W
		return self.function(0x48, 0, RWaction, value, self.limits.b_led_set_max, self.limits.b_led_set_min) # reg_addr, decimal_places, RWaction, value, max_value, min_value

	def model(self):	# R
		return self.function(0x00, 0) #!

	def version(self):	# R
		return self.function(0x03, self.limits.decimals_version)
...
#---		
	...
	def read_all(self, RWaction='r', value=0.0):	# Read data as a block, much faster than individual reads
		data = self.functions(0, 84, RWaction, value) # reg_addr, number of bytes, RWaction, value
		#--- adjust values to floating points
		data[8] = data[8] / float(10**self.limits.decimals_vset)	#100.0	# voltage_set
		data[9] = data[9] / float(10**self.limits.decimals_iset)	#1000.0	# current_set
		data[10] = data[10] / float(10**self.limits.decimals_v)	#100.0	# voltage
		data[11] = data[11] / float(10**self.limits.decimals_i)	#1000.0	# current
		data[13] = data[13] / float(10**self.limits.decimals_power)	#100.0	# power
		data[14] = data[14] / float(10**self.limits.decimals_vin)	#100.0	# voltage_in
		data[3] = data[3] / float(10**self.limits.decimals_version)	#10.0	# version
...
#---		
	...
	def read_all(self, RWaction='r', value=0.0):	# Read data as a block, much faster than individual reads
		data = self.functions(0x08, 16, RWaction, value) # reg_addr, number of bytes, RWaction, value
		#--- adjust values to floating points
...
if __name__ == '__main__':
	ser = Serial_modbus('/dev/ttyUSB1', 1, 115200, 8)
	limits = Import_limits("dps5005_limits.ini")
	dps = DP6012(ser, limits)
	try:
		while True:
